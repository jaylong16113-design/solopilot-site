...
(idx.mood || []).forEach(a => urls.push({ loc: base + "/mood/" + a.slug, priority: "0.7" }));
...
if (fs.existsSync(enIdxPath)) {
  ...
  urls.push({ loc: base + "/en/mood", priority: "0.8" });
  ...
  (enIdx.mood || []).forEach(a => urls.push({ loc: base + "/en/mood/" + a.slug, priority: "0.6" }));
}
